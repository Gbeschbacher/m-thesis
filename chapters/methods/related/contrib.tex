...
Table \ref{tab:transition-table} on page \pageref{tab:transition-table} shows some concepts of animated transitions between visual appearances. Every concept is described with an abbreviation in the table which is explained in detail in the description list below.
...
\begin{table}[!htp]
    ...
    \end{tabular}
    \caption{Transition table showing a transition from a given visualization (column) to any upcoming visualization (rows)}
...
\item The particles simply fade out and the enumeration units gets colored.
\end{enumerate}

\item[Dot] \hfill \\
Dot-based transition means a simple linear transition to the dot map. The desired upcoming visualization is then animated with the dot map as starting point. Therefore this transition implies two transitions.
